                ...
                <form class="row g-3 needs-validation" novalidate enctype="multipart/form-data" method="POST"
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control" id="inventory_id" name="inventory_id" required>
                                ...
                                @foreach($inventory as $item)
                                    <option value="{{ $item->inventory_id }}" {{ old('inventory_id', $forecast->inventory_id) == $item->inventory_id ? 'selected' : '' }}>
                                        {{ $item->inventory_id }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="part_name" name="part_name" value="{{ old('part_name', $forecast->part_name) }}" required readonly>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="part_number" class="form-label">Part Number</label>
                            <input type="text" class="form-control" id="part_number" name="part_number" value="{{ old('part_number', $forecast->part_number) }}" required readonly>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="customer" class="form-label">Customer</label>
                            <input type="text" class="form-control" id="customer" name="customer" value="{{ old('customer', $forecast->customer) }}" required readonly>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="forecast_qty" class="form-label">Forecast Quantity</label>
                            <input type="number" name="forecast_qty" class="form-control" id="forecast_qty" value="{{ old('forecast_qty', $forecast->forecast_qty) }}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="min_stok" class="form-label">Min Stok</label>
                            <input type="number" name="min_stok" class="form-control" id="min_stok" value="{{ old('min_stok', $forecast->min_stok) }}" required>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="max_stok" class="form-label">Max Stok</label>
                            <input type="number" name="max_stok" class="form-control" id="max_stok" value="{{ old('max_stok', $forecast->max_stok) }}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="forecast_date" class="form-label">Forecast Date</label>
                            <input type="date" name="forecast_date" class="form-control" id="forecast_date" value="{{ old('forecast_date', $forecast->forecast_date) }}" required>
                        </div>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.getElementById('inventory_id').addEventListener('change', function () {
                ...
                if (selectedInventory) {
                    ...
                    document.getElementById('customer').value = selectedInventory.customer; // Assuming customer is part of inventory data
                } else {
