...
@section('content')
    ...
    <div class="pagetitle">
        <h1>Data Component Parts</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}">Home</a></li>
                <li class="breadcrumb-item active">Component Parts</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
    @if (session('success'))
                            ...
                            <table class="table table-bordered text-center align-middle datatable">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th scope="col" class="text-center">Area FG</th>
                                ...
                                <tbody>
                                    @foreach ($componentParts as $componentPart)
                                        <tr>
                                            <td class="text-center">{{ $finishedGood->no }}</td>
                                            <td class="text-center">{{ $finishedGood->inventory_id }}</td>
                                            <td class="text-center">{{ $finishedGood->part_name }}</td>
                                            <td class="text-center">{{ $finishedGood->part_number }}</td>
                                            <td class="text-center">{{ $finishedGood->type_package }}</td>
                                            <td class="text-center">{{ $finishedGood->qty_package }}</td>
                                            <td class="text-center">{{ $finishedGood->project }}</td>
                                            <td class="text-center">{{ $finishedGood->customer }}</td>
                                            <td class="text-center">{{ $finishedGood->area_fg }}</td>
                                            <td class="text-center">
                                                <!-- Edit Button (All Data) -->
                                                <a class="btn btn-primary btn-sm ms-1 mt-1 shadow-sm" href="{{ route('finished_goods.edit', ['id' => $finishedGood->id]) }}">
                                                    <i class="bi bi-pencil-square"></i> Edit
                                                </a>

                                                @if ($finishedGood->status)
                                                    <button class="btn btn-success btn-sm ms-1 mt-1 shadow-sm" onclick="changeStatus({{ $finishedGood->id }}, 0)" data-bs-toggle="tooltip" data-bs-placement="top" title="Klik untuk Aktifkan">
                                                        <i class="bi bi-check-circle"></i> Aktif
                                                    </button>
                                                @else
                                                    <button class="btn btn-danger btn-sm ms-1 mt-1 shadow-sm" onclick="changeStatus({{ $finishedGood->id }}, 1)" data-bs-toggle="tooltip" data-bs-placement="top" title="Klik untuk Nonaktifkan">
                                                ...
                                                <script>
                                                    function changeStatus(finishedGoodId, status) {
                                                        ...
                                                        Swal.fire({
                                                            ...
                                                            if (result.isConfirmed) {
                                                                ...
                                                                fetch(`/finished_goods/${finishedGoodId}/change-status`, {
                                                                        method: 'POST',
                                                                        headers: {
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('finished_goods.import') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
